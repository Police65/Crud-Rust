...
async fn index(db: &Products) -> Template {
    ...
    let rows = sqlx::query("SELECT * FROM products").fetch_all(&db.0).await.unwrap();
...
async fn crear_producto(
    form: rocket::form::Form<ProductForm>,
    db: &Products
...
async fn update(
    form: rocket::form::Form<EditProductForm>,
    db: &Products
) -> Result<Redirect, Template> {
    let mut context = HashMap::new();
    let result = sqlx
...
async fn borrar(id: i32, db: &Products) -> Result<Redirect, Template> {
    let mut context = HashMap::new();
    let result = sqlx::query("DELETE FROM products WHERE id = ?").bind(id).execute(&db.0).await;
    match result {
        Ok(_) => { Ok(Redirect::to("/")) }
        Err(_) => {
